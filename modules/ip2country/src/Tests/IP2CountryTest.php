...
class IP2CountryTest extends WebTestBase {
  ...
  /**
   * Implements WebTestBase::getInfo().
   */
  ...
  public function testUserAccess() {
    ...
    $this->assertText(
      t('The IP to Country database has been updated from @rir.', array('@rir' => Unicode::strtoupper($rir)))
    );
    ...
    $this->assertLink(t('Manual database update from @rir server.', array('@rir' => Unicode::strtoupper($rir))));

    // Drill down
    $this->clickLink(t('Manual database update from @rir server.', array('@rir' => Unicode::strtoupper($rir))));
    $this->assertText(t('Details'));
    $this->assertText('ip2country');
    $this->assertText(t('Manual database update from @rir server.', array('@rir' => Unicode::strtoupper($rir))));
...
//    'afrinic' => 'AFRINIC', // Don't use AFRINIC because it's incomplete
    ...
    $this->assertTrue(
      ($count = ip2country_get_count()) != 0,
      t('Database has been updated from %rir with @rows rows.', array('%rir' => Unicode::strtoupper($rir), '@rows' => $count))
    );
