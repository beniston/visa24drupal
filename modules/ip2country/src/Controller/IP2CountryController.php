  ...
  public function updateDatabaseAction($rir) {
    ...
    if ($status != FALSE) {
      if ($ip2country_config->get('watchdog')) {
        \Drupal::logger('ip2country')->notice('Manual database update from @registry server.', array('@registry' => Unicode::strtoupper($rir)));
      }
      print Json::encode(array(
        ...
        'message' => t('The IP to Country database has been updated from @server.',
                       array('@server' => Unicode::strtoupper($rir))),
      ));
    }
    else {
      if ($ip2country_config->get('watchdog')) {
        \Drupal::logger('ip2country')->notice('Manual database update from @registry server FAILED.', array('@registry' => Unicode::strtoupper($rir)));
      }
      print Json::encode(array(
